...
export default {
  ...
  actions: {
    async getFirstSong({ getters, commit }, params) {
      ...
      if (idx === alb.tracks.length) { idx = 0 }
      if (idx < 0) { idx = alb.tracks.length - 1 }
      const url = await firebase
        .storage()
        .ref(`/${id}/`)
        .child(`${alb.tracks[idx].ref}.mp3`)
        .getDownloadURL();
      const song = { albumImg: alb.src, ...alb.tracks[idx], id: alb.id, url, play: true, desc: alb.title };

      commit("setSong", song);
    },
    async nextSong ({getters, dispatch}) {
      const currentAlb = await dispatch('getCurrentAlb')
      const currentSongIdx = currentAlb.tracks.findIndex(t => t.ref === getters.song.ref)
