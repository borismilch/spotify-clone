<template>
  ...
  <section v-else>
    ...
    <v-col cols="12" md="4" style="padding:0px; margin: 20px 0px 20px 0px">
      <router-link to="/genre/metal"  style="height: 220px; padding: 0px; background: rgb(119, 119, 119)" tag="div" class="gategory">
        <h3 class="category-title-big">Метал</h3>
        <img :src="doom" class="gategory-img" alt="">
      </router-link>
    </v-col>
    <h2 class="search-title mt-3">{{searchInCategories}}</h2>
    <div class="search-grid my-6">
      <router-link v-for="c in genres" :key="c.img + c.title" :to="'/genre/' + c.title.toLowerCase()" tag="div" class="gategory" :style="{ background: c.color}">
        <h3 class="category-title font-bold">{{c.title}}</h3>
        <img :src="c.img" style="width:100px; height: 100px" class="gategory-img gategory-img--sm" alt="">
      </router-link>
    </div>
  ...
  computed: {
    ...
    searchingTracks() {
      return (
        this.allTracks
        .filter((t) => {
          return  (this.normalize(t.title).includes(this.normalQuery) || this.normalize(t.album).includes(this.normalQuery) || this.normalize(t.author).includes(this.normalQuery)) 
        })
